...
const webRoot = join(root, "apps", "local", "src", "web");
...
function defaultPackageJson(name: string): string {
...
async function readNotebookPackageJson(name: string): Promise<string> {
  const notebookDir = safeJoin(notebooksRoot, name);
  const packagePath = safeJoin(notebookDir, "package.json");
  if (!existsSync(packagePath)) {
    return defaultPackageJson(name);
  }
  return await readText(packagePath);
}
  ...
  async fetch(req) {
    try {
      ...
      if (url.pathname.startsWith("/api/package/")) {
        const name = decodeURIComponent(url.pathname.replace("/api/package/", ""));

        if (req.method === "GET") {
          return Response.json({ packageJson: await readNotebookPackageJson(name) });
        }

        if (req.method === "PUT") {
          const body = (await req.json()) as { packageJson?: string };
          if (typeof body.packageJson !== "string") {
            return jsonError("Missing packageJson string.", 400);
          }

          try {
            JSON.parse(body.packageJson);
          } catch (error) {
            return jsonError(`Invalid package.json: ${error instanceof Error ? error.message : String(error)}`, 400);
          }

          const notebookDir = safeJoin(notebooksRoot, name);
          await writeText(safeJoin(notebookDir, "package.json"), body.packageJson.trimEnd() + "\n");
          return Response.json({ ok: true, packageJson: await readNotebookPackageJson(name) });
        }
      ...
      if (url.pathname.startsWith("/api/export-html/")) {
        ...
        const html = renderHtmlFragment(cell.body, notebook.cells);
      ...
      if (url.pathname.startsWith("/api/run-cell/")) {
        if (req.method !== "POST") return jsonError("Method not allowed", 405);

        const parts = url.pathname.replace("/api/run-cell/", "").split("/");
        const name = decodeURIComponent(parts[0] ?? "");
        const cellId = decodeURIComponent(parts[1] ?? "");
        const notebook = await loadNotebook(name);
        const cell = findCell(notebook, cellId);

        if (cell.ext !== "ts" && cell.ext !== "js") {
          return jsonError("Only .ts cells can be executed.", 400);
        }

        const active = activeRuns.get(name) ?? 0;
        if (active >= notebook.config.execution.maxConcurrentRuns) {
          return jsonError(
            `Notebook already has ${active} active run(s). maxConcurrentRuns is ${notebook.config.execution.maxConcurrentRuns}.`,
            409,
          );
        }

        activeRuns.set(name, active + 1);

        try {
          const notebookDir = safeJoin(notebooksRoot, name);
          const result = await runTypeScriptCell({
            notebookDir,
            cell,
          ...
          const nextActive = (activeRuns.get(name) ?? 1) - 1;
          if (nextActive <= 0) {
            activeRuns.delete(name);
          } else {
            activeRuns.set(name, nextActive);
          }
        }
      }

      if (url.pathname.startsWith("/api/check-cell/")) {
        if (req.method !== "POST") return jsonError("Method not allowed", 405);

        const parts = url.pathname.replace("/api/check-cell/", "").split("/");
        const name = decodeURIComponent(parts[0] ?? "");
        const cellId = decodeURIComponent(parts[1] ?? "");
        const notebook = await loadNotebook(name);
        const cell = findCell(notebook, cellId);
        ...
        if (cell.ext !== "ts") {
          return jsonError("Only .ts cells can be typechecked.", 400);
        }

        const notebookDir = safeJoin(notebooksRoot, name);
        const result = await checkTypeScriptCell({
          notebookDir,
          cell,
          config: notebook.config,
        });
