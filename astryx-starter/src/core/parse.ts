...
function extFromFilename(filename: string): CellExt {
...
function parseSection(rest: string): Section {
  const parts = rest.trim().split(/\s+/);
  const id = parts[0];

  if (!id || !/^\d{3}$/.test(id)) {
    throw new Error(`Invalid section id "${id}". Expected 010.`);
  }

  return {
    id,
    title: parts.slice(1).join(" ") || `Section ${id}`,
    cellIds: [],
  };
}
...
function parseCellHeader(rest: string): {
  ...
  if (!id || !/^\d{3}\.\d{3}$/.test(id)) {
    throw new Error(`Invalid cell id "${id}". Expected 010.010.`);
  }
