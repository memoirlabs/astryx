...
export function normalizeConfig(source: string): NotebookConfig {
  ...
  return {
    ...
    execution: {
      timeoutMs: Number(raw.tables.execution?.timeoutMs ?? 5000),
      stdoutBytes: Number(raw.tables.execution?.stdoutBytes ?? 1_048_576),
      stderrBytes: Number(raw.tables.execution?.stderrBytes ?? 1_048_576),
      resultBytes: Number(raw.tables.execution?.resultBytes ?? 5_242_880),
      maxConcurrentRuns: Number(raw.tables.execution?.maxConcurrentRuns ?? 1),
    },
  };
}
